class Piece < ActiveRecord::Base
  ...
  def is_obstructed?(x_destination, y_destination)
    ...
    if moving_horizontally?
      return Piece.where(game_id: game.id, y_position: y_start, x_position: (x_start + 1..x_destination - 1)).exists?
    elsif moving_vertically?
      return Piece.where(game_id: game.id, x_position: x_start, y_position: (y_start + 1..y_destination - 1)).exists?
